...
const analyzeAccessibility = (driver) =>
  new Promise((resolve, reject) => {
    AxeBuilder(driver)
      .disableRules(disabledAccessibilityRules)
...
const formatAccessibilityViolations = (violations) =>
  `${violations.length} violations found: ${formatViolationMessages(
    violations
  )}`
...
export const elementCanReceiveFocus = async (element, driver) => {
  ...
  await expect(element.className).to.equal(
    driver.switchTo().activeElement().className
  )
}
