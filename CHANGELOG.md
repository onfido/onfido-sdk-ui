...
### Added
- Public: Added support for Spanish language. The SDK can now be used in Spanish by adding `language: {locale: 'es'}` to options at initialization. If `language` is not present the locale will default to `en`.
